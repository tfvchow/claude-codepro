...
class FinalizeStep(BaseStep):
    ...
    def run(self, ctx: InstallContext) -> None:
        ...
        self._cleanup_legacy_rules(ctx)

        self._install_statusline_config(ctx)

        self._display_success(ctx)

    def _cleanup_legacy_rules(self, ctx: InstallContext) -> None:
        """Remove legacy generated rule files.

        Claude Code now natively loads rules from .claude/rules/*.md files,
        so we no longer need to generate CLAUDE.md and CLAUDE.local.md.
        This cleanup ensures users migrating from older versions don't have
        stale generated files that would bloat context.
        """
        ui = ctx.ui
        cleaned_files: list[str] = []

        # Files that were previously auto-generated by build.py
        legacy_files = [
            ctx.project_dir / ".claude" / "CLAUDE.md",
            ctx.project_dir / "CLAUDE.local.md",
            ctx.project_dir / ".claude" / "rules" / "build.py",
        ]
        ...
        for legacy_file in legacy_files:
            if legacy_file.exists():
                try:
                    legacy_file.unlink()
                    cleaned_files.append(legacy_file.name)
                except (OSError, IOError):
                    pass

        if cleaned_files and ui:
            ui.section("Migrating Rules")
            ui.status("Claude Code now natively loads .claude/rules/*.md files")
            for fname in cleaned_files:
                ui.success(f"Removed legacy file: {fname}")
            ui.info("Rules are now loaded directly without compilation")

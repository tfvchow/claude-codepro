...
def get_alias_line(shell_type: str) -> str:
    ...
    if shell_type == "fish":
        return (
            ...
            "nvm use 22; and clear; and dotenvx run -f .env.codepro -- claude; "
            "else if test -d /workspaces; "
            'set ccp_dir ""; for d in /workspaces/*/; test -d "$d.claude/rules"; and set ccp_dir "$d"; and break; end; '
            'if test -n "$ccp_dir"; cd "$ccp_dir"; and nvm use 22; and clear; and dotenvx run -f .env.codepro -- claude; '
        ...
        return (
            ...
            "nvm use 22 && clear && dotenvx run -f .env.codepro -- claude; "
            "elif [ -d /workspaces ]; then "
            'ccp_dir=""; for d in /workspaces/*/; do [ -d "$d.claude/rules" ] && ccp_dir="$d" && break; done; '
            'if [ -n "$ccp_dir" ]; then cd "$ccp_dir" && nvm use 22 && clear && dotenvx run -f .env.codepro -- claude; '
